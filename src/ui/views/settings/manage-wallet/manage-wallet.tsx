...
} from '@/ui/components/wallet/WalletDisplayName/getWalletDisplayName';
...
function needsBackup(group: WalletGroup) {
  return (
...
function BackupInfoNote({ group }: { group: WalletGroup }) {
  ...
  if (group.origin === WalletOrigin.extension) {
    return (
      <div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-500">
        ...
        <span className="text-[11px] font-medium">Never backed up</span>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap gap-2">
        {wallets.slice(0, 4).map((w) => (
          <button
            ...
            className="flex items-center gap-1 bg-white/50 dark:bg-black/20 px-2 py-0.5 rounded-md hover:bg-white/80 dark:hover:bg-black/40 transition-colors"
          >
            <code className="text-[12px] text-muted-foreground font-mono">
              {copied === w.address ? 'Copied!' : truncateAddress(w.address)}
            </code>
            <FiCopy size={10} className="text-muted-foreground" />
          </button>
        ))}
      </div>
    </div>
...
function WalletGroupCollapsible({
  group,
  index,
}: {
  group: WalletGroup;
          ...
          <div className="px-4 pb-4 pt-3 flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/settings/manage-wallets/groups/${group.id}`);
                }}
