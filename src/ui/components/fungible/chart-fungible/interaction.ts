...
/**
 * Custom interaction mode: when hovering near a data point that has an action
 * (within a magnetic radius), snap to that action point instead of the
 * closest-by-x chart point.
 */
    ...
    (element, datasetIndex, index) => {
      ...
      if (
        ...
        if (!closestPointWithExtra) {
          closestPointWithExtra = { element, datasetIndex, index };
        } else if (
